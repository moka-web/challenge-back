...
Las migraciones TypeORM se encuentran en `src/database/migrations` (ej.: `src/database/migrations/1767677414604-tomasMig.ts`). Las seed migrations suelen ubicarse en subdirectorios, como `src/database/migrations/seed-migrations/`.
...
- `pnpm run create-migration -- <path>`: Crea una nueva migración (usa el CLI de TypeORM).
